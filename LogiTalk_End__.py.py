...
class MainWindow(CTk):
    def __init__(self):
        ...
        # main
        self.chat_field = CTkTextbox(self, font=('Arial', 14, 'bold'), state='disable', fg_color="#595959")
        ...
        self.username = 'Скрипаль Матвій'
        # connect to server
        try:
            self.sock = socket(AF_INET, SOCK_STREAM)
            self.sock.connect(("192.168.0.39", 8080))
            hello = "New user connected!\n"
            self.sock.send(hello.encode('utf-8'))
            threading.Thread(target=self.recv_message, daemon=True).start()
        except Exception as e:
            self.add_message(f"Couldn't connect to server: {e}")
        self.adaptive_ui()
